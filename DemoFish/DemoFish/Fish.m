...
-(void) swimmingFish{
    ...
    float random2 =arc4random()/10000000;
        _fishView.frame =CGRectMake(0  ,random1, _imageFish.size.width, _imageFish.size.height);    [UIView animateWithDuration:4 animations:^{
    
    _fishView.frame =CGRectMake(160,random2, _imageFish.size.width, _imageFish.size.height);
        
    }completion:^(BOOL finished) {
        [UIView animateWithDuration:4 animations:^{
    _fishView.frame =CGRectMake(320,random2, _imageFish.size.width, _imageFish.size.height);
        }completion:^(BOOL finished) {
            [self swimmingFish];
        }];
